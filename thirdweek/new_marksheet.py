def calculate(name, fname, college):
...
while True:
	try:
		...
		dmy = f"{dd}/{mm}/{yyyy}"
		if not name.isalpha() or not fname.isalpha() or not college.isalpha() or not rnumber.isdigit()or not dmy.replace('/', '').isdigit():
			...
			for x in subject:
				...
				if a > 100 or a<=0:print("Invalid marks! Please enter marks out of 100 or grater than 0.")
				exit(1)
				Marks.append(a)
				MTotal += a
				if a >= 80:grades.append("A")
			    elif a >= 60:grades.append("B")
				elif a >= 40:grades.append("C")
			    elif a >= 30:grades.append("D")
